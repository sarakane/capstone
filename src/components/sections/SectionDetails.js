...
function SectionDetails({ match, history }) {
  ...
  const sectionResources = useSelector(state => state.firestore.ordered.resources);

  function handleDeletingSection(sectionId) {
    sectionResources.forEach(resource => firestore.delete({collection: 'libraries', doc: match.params.id, subcollections: [{collection: 'resources', doc: resource.id}]}));
    firestore.delete({collection: 'libraries',  doc: match.params.id, subcollections: [{collection: 'sections', doc: sectionId}]});
  ...
  if(isLoaded(section)) {
    return (
      <>
        ...
        <button onClick={() => handleDeletingSection(section.id)} className="btn deep-purple darken-4">Delete</button>
        <hr />
        <h3>Resources</h3>
